...
import geometry_msgs.msg._Vector3
...
class Server :
    ...
    def pointcloud_callback(self, msg) :
        ...
        #print("point cloud subscribe complete")repub_image_raw
    ...
    def bbox_callback(self,msg) :
        ...
        # print(msg)
        # print("---------------")
        global num_bbox, bbox_arr
        num_bbox = len(msg.boxes)
        bbox_arr = msg
        self.Camera_LiDAR_Projection()
...
def CL_Projection(image_msg,pointcloud_msg):
    ...
    for i in range(0,points.shape[0]) :
        ...
        for k in range(0,num_bbox) :
            if ((xt > bbox_arr.boxes[k].pose.position.x-(bbox_arr.boxes[k].dimensions.x)/2 )and (xt < bbox_arr.boxes[k].pose.position.x + (bbox_arr.boxes[k].dimensions.x)/2)):
                points[i][0]=temp_points[i][0]
            if ((yt > bbox_arr.boxes[k].pose.position.y-(bbox_arr.boxes[k].dimensions.y)/2) and (yt < bbox_arr.boxes[k].pose.position.y + (bbox_arr.boxes[k].dimensions.y)/2)):
                points[i][1]=temp_points[i][1]
            if ((zt > bbox_arr.boxes[k].pose.position.z-(bbox_arr.boxes[k].dimensions.z)/2) and (zt < bbox_arr.boxes[k].pose.position.z + (bbox_arr.boxes[k].dimensions.z)/2)):
    ...
    # print(bbox_arr)
    # print("///")
    # print(bbox_arr.boxes[0].pose.position.x)
...
def Subscriber():
    ...
    rospy.Subscriber("/re_image_raw", Image, server.image_callback)
    rospy.Subscriber("/re_points_raw",PointCloud2,server.pointcloud_callback)
    rospy.Subscriber("/arr_bbox",BoundingBoxArray,server.bbox_callback)
